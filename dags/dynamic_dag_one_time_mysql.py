...
with DAG(
    dag_id="dynamic_dag_one_time_mysql",
    ...
    while start_date < end_date:
        ...
        end_timestamp = start_date.strftime("%Y-%m-%d 23:59:59")
        start_date_display = start_date.strftime("%Y-%m-%d")
        get_dataset_to_s3 = SqlToS3Operator(
            task_id=f"sql_to_s3_task_{start_date_display}",
            ...
            + f"/{start_date_display}/data.parquet",
            replace=True,
        )
        load_data_to_clickhouse = PythonOperator(
            task_id=f"load_task_{start_date_display}",
        ...
        get_dataset_to_s3.set_upstream(t1)
        get_dataset_to_s3 >> load_data_to_clickhouse
